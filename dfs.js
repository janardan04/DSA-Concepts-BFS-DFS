...
function generateGraph() {
    ...
    document.getElementById('stackList').textContent = "Empty";
...
function startDrag(e) {
    isDragging = true;
    currentNode = e.target.closest("circle");
...
function runDFS() {
    ...
    function dfsStep() {
        ...
        if (!visited.has(currentId)) {
            ...
            // Explanation for the node being popped from the stack
            explanation += `Step ${++step}: Popped node ${currentId} from the stack.\n`;
            explanation += `Current stack: [${stack.join(", ")}].\n`;

            const adjacent = getAdjacent(currentId);
            if (adjacent.length === 0) {
                explanation += `Node ${currentId} has no unvisited neighbors.\n`;
            }
            ...
            for (const neighbor of adjacent) {
                if (!visited.has(neighbor) && !stack.includes(neighbor)) {
                    stack.push(neighbor);
                    explanation += `Pushed node ${neighbor} onto the stack as it's unvisited.\n`;
                }
            }

            // Update the node colors based on stack status
            svg.querySelectorAll("circle").forEach(circle => {
                const id = parseInt(circle.getAttribute("data-id"));
                if (id === currentId) {
                    circle.setAttribute("fill", "#4CAF50"); // Mark the current node as visited (green)
                } else if (stack.includes(id)) {
                    circle.setAttribute("fill", "#FFC107"); // Nodes in stack are yellow (will be visited later)
                }
            });

            document.getElementById('explanationText').value = explanation;
            document.getElementById('stackList').textContent = `Stack: ${stack.join(", ")}`;
        }
